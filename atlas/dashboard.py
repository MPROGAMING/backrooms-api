...
<h1>BackroomsGPT Atlas v20</h1><div class=grid><div class=card><h2>{stats.get('documents',0)}</h2>Indexed documents</div><div class=card><h2>{stats.get('snapshots',0)}</h2>Snapshots</div><div class=card><h2>{stats.get('edges',0)}</h2>Graph edges</div><div class=card><h2>{stats.get('projects',0)}</h2>Writer projects</div></div>
<h2>Documents by source</h2><table><tr><th>Source</th><th>Count</th></tr>{src}</table><h2>Telemetry</h2><pre>{escape(str(tele))}</pre><p>Database: <code>{escape(str(stats.get('database_path')))}</code></p></body></html>'''
